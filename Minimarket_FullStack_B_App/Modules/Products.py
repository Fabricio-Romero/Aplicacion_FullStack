...
class ProductosApp:
    def __init__(self, root, es_admin=False):
        ...
        self.tree = ttk.Treeview(
            root,
            columns=("ID", "Nombre", "Precio", "Cantidad", "Cat", "Prov"),
            show="headings",
            height=15
        )
    ...
    def agregar(self):
        ...
        if conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO productos (nombre, precio, cantidad, categoria_id, proveedor_id)
                VALUES (%s, %s, %s,
    ...
    def eliminar(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Eliminar producto?"):
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM productos WHERE id=%s", (product_id,)
                )
                conn.commit()
                conn.close()
                self.cargar_productos()
